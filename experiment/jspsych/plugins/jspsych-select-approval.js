...
jsPsych.plugins['select-approval'] = (function() {
    var plugin = {};
    ...
    plugin.trial = function(display_element, trial) {
      var plugin_id_name = "jspsych-select-approval";
      ...
      // inject CSS for trial (チェックボックスについての調整はここで)
      var cssstr = ".jspsych-select-approval-question { margin-top: 2em; margin-bottom: 2em; text-align: center; }"+
      ".jspsych-select-approval-text span.required {color: darkred;}"+
      ".jspsych-select-approval-horizontal .jspsych-select-approval-text { text-align: center;}"+
      ".jspsych-select-approval-option { line-height: 2; }"+
      ".jspsych-select-approval-horizontal .jspsych-select-approval-option {  display: inline-block; margin-left: 1em;  margin-right: 1em;  vertical-align: top;}"+
      "label.jspsych-select-approval-text input[type='checkbox'] {margin-right: 1em;}"
      display_element.innerHTML = '<style id="jspsych-select-approval-css">' + cssstr + '</style>';
      ...
      for (var i = 0; i < trial.questions.length; i++) {
        ...
        // add question text
        display_element.querySelector(question_selector).innerHTML += '<p id="survey-question" class="' + plugin_id_name + '-text select-approval">' + question.prompt + '</p>';
      ...
      display_element.querySelector('#jspsych-select-approval-next').addEventListener('click', function(){
          for(var i=0; i<trial.questions.length; i++){
            if(trial.questions[i].required){
              if(display_element.querySelector('#jspsych-select-approval-'+i+' input:checked') == null){
                display_element.querySelector('#jspsych-select-approval-'+i+' input').setCustomValidity(trial.required_message);
              } else {
                display_element.querySelector('#jspsych-select-approval-'+i+' input').setCustomValidity('');
              }
            }
          }
      ...
      trial_form.addEventListener('submit', function(event) {
          ...
          for(var index=0; index<trial.questions.length; index++){
            var match = display_element.querySelector('#jspsych-select-approval-'+index);
            var val = [];
